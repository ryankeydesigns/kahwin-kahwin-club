...
export function SiteHeader(){const [open,setOpen]=useState(false);return <><HeartAtmosphere/><nav className="nav"><a className="brand" href="/"><img src="/site-logo.svg" alt="KK"/>Khawin-Khawin.club</a><div className={open?"navlinks open":"navlinks"}>{links.map(([n,h])=><a key={h} href={h} onClick={()=>setOpen(false)}>{n}</a>)}</div><a className="navcta" href="/ai-assistant">开始注册询问 ♡</a><button className="menubtn" aria-label="打开菜单" onClick={()=>setOpen(!open)}>{open?<X/>:<Menu/>}</button></nav></>}
export function SiteFooter(){return <footer><a className="brand" href="/"><img src="/site-logo.svg" alt="KK"/>Khawin-Khawin.club</a><p>Registration × AI × Auspicious Date × Wedding Marketplace</p><div className="footerlinks">{links.map(([n,h])=><a key={h} href={h}>{n}</a>)}</div><small>© 2026 Khawin-Khawin.club · 陈氏书院婚姻注册中心</small></footer>}
